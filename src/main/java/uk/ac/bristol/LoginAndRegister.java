...
public class LoginAndRegister extends Application {
  ...
  public int checkInput() {
    ...
    if (userName.trim().equals("")) {
      new Alert(Alert.AlertType.NONE, "account cannot be empty", new ButtonType[]{ButtonType.CLOSE}).show();
      return -1;
    }
    if ((password.trim()).equals("")) {
      new Alert(Alert.AlertType.NONE, "password cannot be empty", new ButtonType[]{ButtonType.CLOSE}).show();
      return -1;
    }
    return 0;
  }
  ...
  private void UserLogin(String userName, String password) throws URISyntaxException, IOException {
    ...
    for (int i = 0; i < FileOperator.userList.size(); i++) {
      if (FileOperator.userList.get(i).getUsername().trim().equals(userName)
        ...
        //close stage
        //open new window
        new Alert(Alert.AlertType.NONE, "login success", new ButtonType[]{ButtonType.CLOSE}).show();
        App app =new App();
        app.loadMain(stage);
      }
    ...
    if (User == null) {
      new Alert(Alert.AlertType.NONE, "account or password is incorrect!", new ButtonType[]{ButtonType.CLOSE}).show();
      user_name.setText("");
  ...
  private void UserRigister(String userName, String password) {
    ...
    for (int i = 0; i < FileOperator.userList.size(); i++) {
      if (FileOperator.userList.get(i).getUsername().trim().equals(userName)
              && FileOperator.userList.get(i).getPassword().trim().equals(password)) {
        User = FileOperator.userList.get(i);
        new Alert(Alert.AlertType.NONE, "user already exists. Please re-register", new ButtonType[]{ButtonType.CLOSE}).show();
        user_name.setText("");
        user_password.setText("");
      }
    }
    ...
    if (User == null) {
      ...
      new Alert(Alert.AlertType.NONE, "register successfully, please login", new ButtonType[]{ButtonType.CLOSE}).show();
      user_name.setText("");
      user_password.setText("");
    }
  }
  ...
  public boolean informationDialog(Alert.AlertType alterType, String title, String header, String message) {
    ...
    Alert alert = new Alert(alterType, message, new ButtonType("cancle", ButtonBar.ButtonData.CANCEL_CLOSE), new ButtonType("yes", ButtonBar.ButtonData.YES));
