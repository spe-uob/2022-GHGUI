...
public class MergeController implements Initializable {
  ...
  @FXML private ComboBox<String> pathComBox;
  ...
  void gitMerge(ActionEvent event) {
    if (pathComBox.getSelectionModel().getSelectedItem() != null) {
      ...
      try {
        branchId =
            gitInfo.getRepo().resolve(pathComBox.getSelectionModel().getSelectedItem().toString());
        MergeCommand mergeCommand =
            gitInfo.command(Git::merge).include(branchId).setStrategy(MergeStrategy.RECURSIVE);
